...
$query = "INSERT INTO job_types(name) VALUES('$name')";
if (!mysqli_query($conn, $query)) {
  $error = "DB Error: " . mysqli_error($conn);

} else {
  $success = "Job Types added successfully.";
}
...
if (isset($_GET['delete'])) {
    ...
    if (!mysqli_query($conn, $query)) {
        $error = "Cannot delete this Job Types, to delete this first delete jobs that associated with it";
      
    } else {
      $success = "Job Types deleted successfully.";
    }
}
...
if (isset($_GET['edit'])) {
  $id = $_GET['edit'];
  $edit_jobtype = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM job_types WHERE id = $id"));
}
...
if (isset($_POST['update'])) {
    $id = $_POST['id'];
    $name = $_POST['name'];
    $query = "UPDATE job_types SET name = '$name'";
    $query .= " WHERE id = $id";

    if (!mysqli_query($conn, $query)) {
      $error = "DB Error: " . mysqli_error($conn);
    } else {
      $success = "Job Types updated successfully.";
    }
  header("Job_types: jobtype.php");
}
            ...
            <div class="row">
                ...
                <div class="col-sm-10 col-md-6 col-lg-6 col-xl-6"> <br>
                    ...
                    <div class="form-head">
                        ...
                        <h2><?= $edit_jobtype ? 'Edit' : 'Add' ?> Job Types</h2>
                    </div>    
                    <form method="POST" enctype="multipart/form-data">

                        <input type="hidden" name="id" value="<?= $edit_jobtype['id'] ?? '' ?>">
                        <div class="mb-3 mt-3">
                            <label for="name" class="form-label">Name:</label>
                            <input type="text" name="name" placeholder="job type Name" value="<?= $edit_jobtype['name'] ?? '' ?>" class="form-control" id="name" required >
                        ...
                        <div class="">
                            <button type="submit" name="<?= $edit_jobtype ? 'update' : 'add' ?>" class="btn btn-primary btn-lg">
                            <?= $edit_jobtype ? 'Update' : 'Add' ?>
                            </button> &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;
                    ...
                    <div class="table-responsive table-wrapper">
                        <table class="table table-bordered" border="1" cellpadding="8">
                          ...
                          <?php
                          $res = mysqli_query($conn, "SELECT * FROM job_types");
                          ...
                          ?>
                            <tr>
                              ...
                              <td>
                                ...
                                <a href="?delete=<?= $row['id'] ?>" onclick="return confirm('Delete this jop type?')"  class="btn btn-danger btn-lg">Delete</a>
                              </td>
                            </tr>
                          <?php endwhile; ?>
                        </table> <br>
